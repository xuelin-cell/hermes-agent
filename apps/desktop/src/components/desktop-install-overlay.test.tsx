...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    ...
    expect(await screen.findByText('Set up UniWork Desktop')).toBeTruthy()
    expect(screen.getByText('Connect to existing UniWork')).toBeTruthy()
    expect(screen.getByText('Install UniWork locally')).toBeTruthy()
    expect(screen.queryByText(/steps complete/i)).toBeNull()
    expect(screen.queryByText(/Fetching installer manifest/i)).toBeNull()
  })

  it('continues local bootstrap only when Install UniWork locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install UniWork locally'))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText('Set up UniWork Desktop')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up UniWork Desktop')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install UniWork locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart UniWork Desktop and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install UniWork locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart UniWork Desktop and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install UniWork locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart UniWork Desktop and try again.')
    ).toBeTruthy()
    ...
    expect(screen.queryByText('Local installation could not start. Restart UniWork Desktop and try again.')).toBeNull()
  })
  ...
  it('opens the remote connection form from the first-run choice', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect to existing UniWork'))
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    ...
    expect(await screen.findByText('UniWork needs a one-time install')).toBeTruthy()
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('UniWork needs a one-time install')).toBeNull()
  })
})
